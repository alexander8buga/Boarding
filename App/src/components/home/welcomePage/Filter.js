...
export default class Filter extends Component {
  ...
  handleClick() {
    if(this.state.value !== "" && this.state.value !== null) {
      this.props.history.push({pathname: '/searchedlistengs', state: { message: this.state.value, minPrice: this.state.minPrice, maxPrice:this.state.maxPrice, sortValue:this.state.sortValue}});
    }
  }
      ...
      <div>
        <form  onSubmit={this.handleSubmit}>
          <div className="card" style={{padding: "20px", margin: "10px"}}>
            <span className="input-group-addon" style={{background: "white",  color: "#1995dc", border: "none"}}>Search By City:</span>
            <Autosuggest
            ...
            <span className="input-group-addon" style={{background: "white", fontsize: "18px", color: "#1995dc", border: "none"}}>Type the Price:</span>
            ...
            <button type="submit" className="btn btn-primary btn-block"
             onClick={(this.state.value == "" || this.state.value == null)? () => alert('You need to enter something to search!') : this.handleClick}   >Filter</button>
          </div>
          <br/>
          <br/>
          <div className="card" style={{padding: "20px", margin: "10px"}}>
...
module.exports = withRouter(Filter);
